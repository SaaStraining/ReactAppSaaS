...
const ServiceForm = ({ nextStep, prevStep, handleChange, values }) => {
  ...
  const getServices = () => {
    const iddvis = localStorage.getItem("id_devis");
    axiosInstance.get("/service/get/" + iddvis).then((res) => {
      setServices(res.data);
      console.log("hah"+services);
    });
  };
    ...
    <div className="w-full overflow-auto ">
      <div className="w-5/6 mx-auto  mt-4 mb-7 ">
        <form
          action=""
          onSubmit={addService}
          className="flex mx-auto justify-around"
        >
          <div className="w-56 mr-1 ">
            <label
              htmlFor=""
              className="block text-md font-light mb-4 float-left  "
            >
              Libelle:
            </label>
            <input
          ...
          <div className="w-24 mr-1">
            ...
            >
              <option value="kg" className="  text-center mr-2">
                Kg
              </option>
              <option value="Cm" className="  text-center mr-2">
                Cm
              </option>
              <option value="unite" className=" text-center mr-2">
                Unite
              </option>
              <option value="m" className=" text-center mr-2">
                m
              </option>
            </select>
          </div>
      ...
      <div className=" w-5/6 border flex flex-col  items-start justify-between  mt-10 mb- mx-auto ">
        <p className=" block text-sm mb-2 float-left">
          Total Ht services (prévus):{" "}
          <span className="ml-4">{90454 + 12443497}</span>{" "}
        </p>
        <p className="block text-sm float-left ">
          Total des marges (prévus):{" "}
          <span className="ml-4">{200999 + 123838}</span>{" "}
        </p>
      </div>
      <br />
      <div className="w-5/6 mx-auto float-right">
        <button
          className="w-36 rounded-md px-3 py-2 bg-white text-gray-700"
          onClick={Previous}
        >
          Retour
        </button>
        <button
          className="w-36 rounded-md px-3 py-2 bg-primary text-white "
          onClick={Next}
        >
          Suivant
        </button>
      ...
      <PDFDownloadLink document={<DevisDoc />} fileName="Devis">
        {({ loading }) =>
          loading ? (
            <button>Loading devis file</button>
          ) : (
